...
structured profile with Groq, asks only for missing information, and matches
AI-detected subcategories against the current DatabaseManager catalogue.
"""
...
async def extract(
    ...
    catalog_text = json.dumps(catalog[:320], ensure_ascii=False)
    ...
    messages = [
        {
            "role": "system",
            "content": """You are the partner-onboarding AI for Armenia AI Guide.
Extract a business profile from natural Armenian, Russian or English. Do not invent facts. Merge the new message with the conversation history. Determine the best existing master direction and subcategories from the supplied catalog. Extract every service and price explicitly mentioned.
...
ready=true only when business_name, city, direction and at least one service are known. missing should contain only the still-required fields. Preserve previously extracted fields even when the new message contains only a short answer.""",
        },
        {
            "role": "user",
...
def missing_question(data: dict, lang: str) -> str:
    ...
    lang_questions = questions.get(lang, questions["ru"])
    return lang_questions.get(field, lang_questions["services"])

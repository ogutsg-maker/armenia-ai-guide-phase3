...
def _partner_state(uid: int) -> FSMContext:
    # One conversation state per Telegram user. The state is used only for the
    # AI conversation; catalogue selection is never stored in the FSM.
...
async def _process_partner_onboarding_text(uid: int, text: str, state: FSMContext) -> dict:
    """The active AI registration conversation.

    Persistence is delegated to ai_first_partner_onboarding, so this function
    never creates an approved partner or writes legacy master_skills.
    """
    ...
    missing = [key for key in ("business_name", "city", "direction", "services") if not profile.get(key)]
    ...
    if missing:
        ...
        return {
            "message": t(lang,
                "🤖 Ես արդեն հավաքել եմ ձեր ասած տվյալները։ " + question,
                "🤖 Я уже собрал данные. " + question,
                "🤖 I have collected the information. " + question),
            "completed": False,
            "profile": profile,
        }
    ...
    if result.get("proposal_created"):
        message = t(lang,
            "✅ Տվյալները պահպանված են։ Ձեր ուղղությունը նոր է կամ դեռ չկա կատալոգում։ AI-ը կազմել է առաջարկը և ուղարկել է ադմինիստրատորի հաստատմանը։",
            "✅ Данные сохранены. Ваше направление новое или пока отсутствует в каталоге. AI сформировал предложение и отправил его администратору на рассмотрение.",
            "✅ Your data is saved. The direction is new or not yet in the catalogue. AI created a proposal and sent it to the administrator for review.")
    else:
        message = t(lang,
    ...
    try:
        with db._connect() as conn:
            with conn.cursor() as cur:
                cur.execute("""SELECT pd.id,pd.status,pd.rejection_reason,m.name_am,m.name_ru,
                                      (SELECT COUNT(*) FROM partner_verification_documents d WHERE d.partner_direction_id=pd.id) AS document_count
                               FROM partner_directions pd JOIN master_categories m ON m.id=pd.master_category_id
                               WHERE pd.partner_id=%s AND pd.status<>'deleted' ORDER BY pd.id DESC LIMIT 1""", (partner["id"],))
                direction = cur.fetchone()
    except Exception:
        logger.exception("Could not read partner direction status")
    return web.json_response({
        "ok": True,
        "registered": True,
...
async def cmd_start(message: types.Message, state: FSMContext):
    ...
    if role == "master":
        await message.answer(
            "🏢 Armenia AI Guide\n\nԲացեք գործընկերոջ AI բաժինը՝ ձեր բիզնեսը կառավարելու կամ շարունակելու գրանցումը։",
            reply_markup=_partner_keyboard(),
        )
        return
    if role == "client":
        await message.answer(
            "👤 Armenia AI Guide\n\nԲացեք AI օգնականը՝ ծառայություն գտնելու, ընտրելու և ամրագրելու համար։",
...
async def telegram_partner_auth_middleware(request: web.Request, handler):
    path = request.path
    if not path.startswith("/api/master/") or request.method == "OPTIONS":
...
async def main():
    ...
    # runtime_platform_bootstrap registers current client, marketplace,
    # directions, admin-AI, premium, reviews, support and storefront APIs.

...
def create_partner_application_draft(db, uid: int, profile: dict[str, Any]) -> dict[str, Any]:
    ...
    # AI classification is allowed during registration, but it is only
    # stored on the application as a proposal. It must NOT create an active
    # partner direction/category before Admin approval.
    master_id = _safe_int(profile.get("master_category_id") or profile.get("ai_master_category_id"))
    category_id = None
    if services:
        category_id = _safe_int(
            services[0].get("matched_subcategory_id")
            or services[0].get("subcategory_id")
            or services[0].get("category_id")
        )
    payload = dict(profile)
    payload["services"] = services
    payload["ai_master_category_id"] = master_id
